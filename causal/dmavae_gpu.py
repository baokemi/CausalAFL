...
class NormalNet(DistributionNet):
    ...
    def forward(self, x):
        ...
        # return loc, scale
        return loc.to(x.device), scale.to(x.device)
...
# Guide 类用于定义变分分布，它通过一个神经网络来逼近后验分布。
class Guide(PyroModule):
    def __init__(self, config, device):
        ...
        self.treatment_values = config["treatment_values"]
        
        OutcomeNet = DistributionNet.get_class(config["outcome_dist"])
        super().__init__()
        self.t_nn = BernoulliNet([config["latent_Ztm_dim"] + config["latent_Zty_dim"]]).to(device)

        self.m_nn = {}
        self.y_nn = {}
        for t in self.treatment_values:
            self.m_nn[t] = FullyConnected([config["latent_Ztm_dim"] + config["latent_Zmy_dim"]] +
            ...
            self.y_nn[t] = FullyConnected([config["latent_Zty_dim"] + config["latent_Zmy_dim"] + 1] +
                                          [config["hidden_dim"]] * (config["num_layers"] - 1),
                                          final_activation=nn.ELU())
    ...
    def m_dist(self, t, zmy, ztm):
        ...
        t = t.unsqueeze(-1)
        params = [torch.where(t.bool(), p1, p0) for p0, p1 in zip(params0, params1)]
...
class Model(PyroModule):
    def __init__(self, config, device):
        ...
        self.zmy_nn = FullyConnected([config["feature_dim"]] + [config["hidden_dim"]] * (config["num_layers"] - 1),
                                     final_activation=nn.ELU()).to(device)
        self.ztm_nn = FullyConnected([config["feature_dim"]] + [config["hidden_dim"]] * (config["num_layers"] - 1),
                                     final_activation=nn.ELU()).to(device)
        self.zty_nn = FullyConnected([config["feature_dim"]] + [config["hidden_dim"]] * (config["num_layers"] - 1),
                                     final_activation=nn.ELU()).to(device)
        self.zmy0_nn = DiagNormalNet([config["hidden_dim"], config["latent_Zmy_dim"]]).to(device)
        self.ztm0_nn = DiagNormalNet([config["hidden_dim"], config["latent_Ztm_dim"]]).to(device)
        self.zty0_nn = DiagNormalNet([config["hidden_dim"], config["latent_Zty_dim"]]).to(device)
    ...
    def forward(self, x, m=None, t=None, y=None, size=None):
        ...
        with pyro.plate("data", size, subsample=x):
            ...
            t = self.t_dist(ztm, zty)
            print(t)
            m = self.m_nn[t](ztm, zmy)
            y = self.y_nn[t](ztm, zmy)  
        return y
    ...
    def y_mean(self, x, m, t=None):
        with pyro.plate("data", x.size(0)):
            ...
            for t_val in self.treatment_values:
                m_zmy = torch.cat([m.unsqueeze(-1), zmy], dim=-1)
                m_zmy_zty = torch.cat([m_zmy, zty], dim=-1)
                params = self.y_nn[t_val](m_zmy_zty)
                y_means.append(params)

            return torch.stack(y_means, dim=-1)
    ...
    def m_mean(self, x, t=None):
        with pyro.plate("data", x.size(0)):
            ...
            m_means = []
            for t_val in self.treatment_values:
                m_zmy_ztm = torch.cat([zmy, ztm], dim=-1)
                hidden = self.m_nn[t_val](m_zmy_ztm)
                params = self.m_nn[t_val](hidden)
                m_means.append(params)

            return torch.stack(m_means, dim=-1)
    ...
    def x_dist(self, zmy, ztm, zty):
        ...
        loc, scale = self.to_device(loc, scale)
...
class DMA_VAE(nn.Module):
    def __init__(self, feature_dim, outcome_dist="normal",
                 latent_Ztm_dim=0, latent_Zty_dim=0, latent_Zmy_dim=0, hidden_dim=0, num_layers=0, num_samples=0, device='cuda', treatment_values=[]):
        config = dict(feature_dim=feature_dim, latent_Ztm_dim=latent_Ztm_dim, latent_Zty_dim=latent_Zty_dim, latent_Zmy_dim=latent_Zmy_dim,
                      ...
                      num_samples=num_samples, treatment_values=treatment_values)
        ...
        self.guide = Guide(config, device)
    ...
    def fit(self, x, m, t, y,
        ...
        self.whiten = PreWhitener(x, self.device)
        self.whiten.loc = self.whiten.loc.to(self.device)
        self.whiten.inv_scale = self.whiten.inv_scale.to(self.device)
        ...
        for epoch in range(num_epochs):
            for x, m, t, y in dataloader:
                x = self.whiten(x)
                size = len(dataset)
                loss = svi.step(x, m, t, y, size) / len(dataset)
